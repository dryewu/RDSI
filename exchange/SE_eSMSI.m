function SE_eSMSI(fdwi,fbval,fmask,outpath,options)
    %{
    ░█▀▀█ ░█▀▀▀█ ░█▀▄▀█ ░█▀▀▀ ░█▀▀▄ ▀█▀
    ░█─── ░█──░█ ░█░█░█ ░█▀▀▀ ░█─░█ ░█─
    ░█▄▄█ ░█▄▄▄█ ░█──░█ ░█▄▄▄ ░█▄▄▀ ▄█▄
    ...
    kernel_semi_restricted   = cell(1,num_semi_restricted);
    kernel_free       = cell(1,num_free);
    ...
    for j = 1:num_semi_restricted
        kernel_semi_restricted{1,j} = single(zeros(length(SE_bshell),1));
        for k = 1:length(SE_bshell)
            R = sqrt(SE_bshell(k)*(adc_semi_restricted(j,1)-adc_semi_restricted(j,2)));
            kernel_semi_restricted{1,j}(k,1) = (sqrt(pi)*exp(-SE_bshell(k)*adc_semi_restricted(j,2))*erf(R))/(2*R);
        end
    end
    ...
    for j = 1:num_free
        kernel_free{1,j} = single(zeros(length(SE_bshell),1));
        for k = 1:length(SE_bshell)
            kernel_free{1,j}(k,1) = exp(-SE_bshell(k)*adc_free(j)); 
        end
    end
    
    blk_kernel = cell2mat([kernel_restricted kernel_hindered kernel_semi_restricted kernel_isotropic kernel_free]);
    ...
    alpha_coef = zeros(num_restricted+num_hindered+num_semi_restricted+num_isotropic+num_free,size(SE_dwi_mean_array,2));
    ...
    temp2 = single(zeros(num_semi_restricted,length(SE_dwi_mean_array_ind)));
    temp2(:,SE_dwi_mean_array_ind) = alpha_coef(1+num_restricted+num_hindered:num_restricted+num_hindered+num_semi_restricted,:);
    temp = single(zeros(num_semi_restricted,size(SE_mask,1)*size(SE_mask,2)*size(SE_mask,3)));
    temp(:,SE_mask_ind) = temp2;
    vf = reshape(temp',size(SE_mask,1),size(SE_mask,2),size(SE_mask,3),num_semi_restricted);

    info_vf= SE_dwi_info;
    info_vf.Datatype = 'single';
    info_vf.ImageSize = size(vf);
    niftiwrite(vf,fullfile(outpath,'VF_semi_restricted.nii'),info_vf,'Compressed', true);

    % save VF isotropic
    temp2 = single(zeros(num_isotropic,length(SE_dwi_mean_array_ind)));
    temp2(:,SE_dwi_mean_array_ind) = alpha_coef(1+num_restricted+num_hindered+num_semi_restricted:num_restricted+num_hindered+num_semi_restricted+num_isotropic,:);
    ...
    % save VF isotropic
    temp2 = single(zeros(num_free,length(SE_dwi_mean_array_ind)));
    temp2(:,SE_dwi_mean_array_ind) = alpha_coef(1+num_restricted+num_hindered+num_semi_restricted+num_isotropic:end,:);
    temp = single(zeros(num_free,size(SE_mask,1)*size(SE_mask,2)*size(SE_mask,3)));
    temp(:,SE_mask_ind) = temp2;
    vf = reshape(temp',size(SE_mask,1),size(SE_mask,2),size(SE_mask,3),num_free);
