function demo_ME_SI()
    ...
    ME_vf           = round(niftiread(ME_vf_info));
    ME_t2r          = round(niftiread(ME_t2r_info));
    params = load(fparams);
    ...
    for i = 1:length(TE)
        ...
        ME_bval_norm{i} = ME_bval_norm{i}(~ind_S0,:);
        ME_bvec_norm{i} = ME_bvec_norm{i}(~ind_S0,:);
    end
    ...
    for i = 1:length(TE)
        ...
        for j = 1:num_restricted
            kernel_restricted{i,j} = single(zeros(nvol,1),nmax);
            for k = 1:length(bshell)
                order = floor(nsh2lmax(sum(bval==bshell{k})));
                DW_scheme = gen_scheme(bvec(bval==bshell{k},:),order);
                
                R_amp = response(adc_restricted(j,1),adc_restricted(j,2),bval(bval==bshell{k}),scheme);
        ...
        for j = 1:num_hindered
            kernel_hindered{i,j} = single(zeros(nvol,1),nmax);
            for k = 1:length(bshell)
                order = floor(nsh2lmax(sum(bval==bshell{k})));
                DW_scheme = gen_scheme(bvec(bval==bshell{k},:),order);
                
                R_amp = response(adc_hindered(j,1),adc_hindered(j,2),bval(bval==bshell{k}),scheme);
                R_SH = amp2SH(R_amp, scheme);
                R_RH = SH2RH(R_SH);
        ...
        for j = 1:num_isotropic
            kernel_isotropic{i,j} = single(zeros(nvol,1),1);
            for k = 1:length(bshell)
                order = floor(nsh2lmax(sum(bval==bshell{k})));
                DW_scheme = gen_scheme(bvec(bval==bshell{k},:),order);
                
                R_amp = response(adc_isotropic(j),adc_isotropic(j),bval(bval==bshell{k}),scheme);
                R_SH = amp2SH(R_amp, scheme);
                R_RH = SH2RH(R_SH);
    ...
    ME_vf_array = reshape(ME_vf,size(ME_vf,1)*size(ME_vf,2)*size(ME_vf,3),3);
    ...
    A1 = blkdiag(ampbasis,diag(scheme.sh(1)*ones(1,num_isotropic)));
    ...
    temp(:,ME_mask_ind) = alpha_coef(:,1+num_restricted+num_hindered:end);
    fod = reshape(temp',size(ME_mask,1),size(ME_mask,2),size(ME_mask,3),num_isotropic);
    niftiwrite(fod,fullfile(dataFolder,'FOD_isotropic_.nii'),info_fod,'Compressed', true);
end
